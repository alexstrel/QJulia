...
@inline function gen_unit_spinor!(spin::Vector{Complex{T}}) where T <: AbstractFloat
  field_volume = sqrt(2.0*Float64(length(spin)))
  for i in 1:length(spin);  spin[i] = Complex{T}( 1.0/field_volume , 1.0/field_volume ); end
end #gen_random_spinor!
