...
function solver(x::AbstractArray, b::AbstractArray, Mat::Any, MatSloppy::Any, param::QJuliaSolvers.QJuliaSolverParam_qj, K::Function, extra_args...)
    ...
    println(solver_name," : Initial preconditioned residual ", norm(u))
    ...
    # Main loop:
    max_res_incr = 32
    aux_rel_trigger = max_res_incr
    while (k < param.maxiter && converged == false && aux_rel_trigger > 0 )
      ...
      if(rNormOld < rNorm); aux_rel_trigger -= 1; end
