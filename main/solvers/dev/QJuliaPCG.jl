...
#    local u       = is_preconditioned == true ? typeof(rSloppy)(undef, length(rSloppy)) : rSloppy
    ...
    local w       = typeof(rSloppy)(undef, length(rSloppy))
    ...
    γnew = rdot(rSloppy, u); γold = 0.0; γt = 0.0; β = 0.0
    δ    = rdot(u, w); α = γnew / δ
...
@time    while (k < param.maxiter && converged == false)
      ...
      β  =  γaux / γold
      # update α
      #α  =  γnew / (δ+β*(rdot(u,s)+rdot(p,w))+β*β*rdot(p,s))
      αold = α
      α = γnew / (δ-(β*γnew)/αold)
...
function solver1(x::AbstractArray, b::AbstractArray, Mat::Any, MatSloppy::Any, param::QJuliaSolvers.QJuliaSolverParam_qj, K::Function)
...
function solver2(x::AbstractArray, b::AbstractArray, Mat::Any, MatSloppy::Any, param::QJuliaSolvers.QJuliaSolverParam_qj, K::Function)
...
function solver3(x::AbstractArray, b::AbstractArray, Mat::Any, MatSloppy::Any, param::QJuliaSolvers.QJuliaSolverParam_qj, K::Function)
